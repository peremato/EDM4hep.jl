...
H1D(title, nbins, min, max; unit=:nounit) = H1D(title, Hist1D(Float64; bins=range(min,max,nbins+1), overflow=false), unit, _getvalue(unit))

Base.push!(h::H1D, v, w=1) = push!(h.hist, v/h.uval, w)
...
H2D(title, xbins, xmin, xmax, ybins, ymin, ymax; units=(:nounit, :nounit)) = H2D(title, Hist2D(Float64;bins=(range(xmin,xmax,xbins+1), range(ymin,ymax, ybins+1)), overflow=true), units, _getvalue(units))

Base.push!(h::H2D, u, v, w=1) = push!(h.hist, u/h.uval[1], v/h.uval[2], w)
...
H3D(title, xbins, xmin, xmax, ybins, ymin, ymax, zbins, zmin, zmax; units=(:nounit, :nounit, :nounit)) = 
   H3D(title, Hist3D(Float64;bins=(range(xmin,xmax,xbins+1), range(ymin,ymax, ybins+1), range(zmin,zmax, zbins+1)), overflow=true), units, _getvalue(units))

Base.push!(h::H3D, x, y, z, w=1) = push!(h.hist, x/h.uval[1], y/h.uval[2], z/h.uval[3], w)
